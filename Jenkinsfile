            ...
            steps {
                script {
                    ...
                    sshagent(['tomcat']) {
                        ...
                        sh returnStatus: true, script: "ssh -o StrictHostKeyChecking=no ubuntu@18.237.141.41 ${stop_container}"
                        sh returnStatus: true, script: "ssh -o StrictHostKeyChecking=no ubuntu@18.237.141.41 ${delete_contName}"
                        sh returnStatus: true, script: "ssh -o StrictHostKeyChecking=no ubuntu@18.237.141.41 ${delete_images}"

                    // Change the IP address of the production server
                        sh "ssh -o StrictHostKeyChecking=no ubuntu@18.237.141.41 ${image_run}"
